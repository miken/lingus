...
Meteor.methods
  post: (lingoProperties) ->
    ...
    throw new Meteor.Error(422, "Cannot add a blank lingo")  unless lingoProperties.name
    
    # check that there are no duped lingos
    throw new Meteor.Error(302, "This lingo already exists", lingoAlreadyExists._id)  if lingoProperties.name and lingoAlreadyExists
  ...
  update: (currentLingoId, lingoProperties) ->
    ...
    # ensure the lingo has a name
    throw new Meteor.Error(422, "Cannot leave a lingo blank")  unless lingoProperties.name
    ...
    lingoId = Lingos.update(currentLingoId,
      $set: lingoProperties
    )
    lingoId